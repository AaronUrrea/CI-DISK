...
void testReadWrite()
{
   ...
   for (int i = 0; i < numberOfTrials; i++)
   {
      ...
      if ((errorCode = writeDisk(lba, writeBuffer)) != CIDEV_SUCCESS)
      {
         ...
         // TODO - PLEASE READ PROFESSOR
         //  Having this free here would cause a double free situation in which
         //  the program would throw a "corrupted size vs prev_size" error, and
         //  then promptly abort. I have commented it out as through all of my trials,
         //  having this code removed prevented that error, while still freeing the
